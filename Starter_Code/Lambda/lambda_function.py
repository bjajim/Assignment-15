...
def recommend_portfolio(intent_request):
    ...
    if age == None:
        ...
        message = {"contentType": "PlainText", "content": "Not a valid age, please what is your age?"}
        return elicit_slot(None, intent_request["currentIntent"]["name"], get_slots(intent_request), "age", message)
    elif parse_int(age) >= 65:
        message = {"contentType": "PlainText", "content": "You can not be 65 or older, what is your age?"}
        return elicit_slot(None, intent_request["currentIntent"]["name"], get_slots(intent_request), "age", message)
    
    if investment_amount == None:
        return delegate({}, get_slots(intent_request)) 
    elif parse_int(investment_amount) < 5000:
        message = {"contentType": "PlainText", "content": "You can not invest less than 5000, how much will you like to invest?"}
        return elicit_slot(None, intent_request["currentIntent"]["name"], get_slots(intent_request), "investmentAmount", message)
    
    if risk_level == None:
        return delegate({}, get_slots(intent_request)) 
    
    message_content = "I reccommend for you :  "
    if risk_level.lower() == "none":
        message_content +=  "100% bonds (AGG), 0% equities (SPY)"
    elif risk_level.lower() == "low":
        message_content +=  "60% bonds (AGG), 40% equities (SPY)"
    elif risk_level.lower() == "medium":
        message_content +=  "40% bonds (AGG), 60% equities (SPY)"
    if risk_level.lower() == "high":
        message_content +=  "20% bonds (AGG), 80% equities (SPY)"
    
    return close(None, "Fulfilled", {"contentType": "PlainText", "content": message_content})
